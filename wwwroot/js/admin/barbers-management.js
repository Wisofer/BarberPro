 ...
 * Maneja todas las funcionalidades de la página de gestión de barberos
 */
...
(function() {
    ...
    function toggleBarberStatus(id, isActive) {
        var message = "¿Estás seguro de " + (isActive ? "desactivar" : "activar") + " este barbero?";
        if (!confirm(message)) {
            return;
        }
    ...
    function confirmDeleteBarber(id, name) {
        document.getElementById("barberNameToDelete").textContent = name || "este barbero";
    ...
    function deleteBarber() {
        ...
        if (!id) {
            alert("Error: No se pudo identificar el barbero a eliminar");
            return;
        }
    ...
    function exportToExcel() {
        ...
        link.download = "barberos_" + dateStr + ".csv";
        link.click();
    }
